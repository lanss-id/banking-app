...
export const signIn = async ({ email, password }: signInProps) => {
  try {
    ...
    console.error("Error Sign In Server Actions:", error);
  }
};
...
export const exchangePublicToken = async ({
  ...
  try {
    ...
    const accessResponse = await plaidClient.accountsGet({
      access_token: accessToken,
    });

    const accountData = accessResponse.data.accounts[0];
    ...
    // If founding Source is not created, throw an error
